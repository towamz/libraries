...
Public Function getFilenamesArray()
	...
	ReDim aryFileName(objFiles.count)
...
Public Function getFilenamesDictionary()
	...
	With CreateObject("VBScript.RegExp")
		...
		For Each objFile in objFiles
			If .Test(objFile.name) then
				dicFileNames.Add objFile.name,objFile.Path
			End If 
		Next
	End With
...
Public Function getFilenamesDictionaryMany(argNum)
	Dim objFiles, objFile
	Dim aryDics()
	Dim dicFileNames
	Dim cntIndex

	'引数が０以上かチェック / check if the argument is greater than 0
	If Not IsNumeric(argNum) Then
		Err.raise 1000
    ...
    ReDim aryDics(argNum)

    for cntIndex = 0 to argNum
        set aryDics(cntIndex) = CreateObject("Scripting.Dictionary")
    Next
	...
	With CreateObject("VBScript.RegExp")
		...
		For Each objFile in objFiles
			If .Test(objFile.name) then
				for cntIndex = 0 to argNum
					aryDics(cntIndex).Add objFile.name,objFile.Path
				next
			End If 
		Next
	End With

	getFilenamesDictionaryMany = aryDics
